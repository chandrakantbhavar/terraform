...
func resourceAwsWafIPSetUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	err := updateDescriptors(d.Id(), oldD, newD, conn)
	if err != nil {
...
func resourceAwsWafIPSetDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err := updateDescriptors(d.Id(), oldDescriptors, noDescriptors, conn)
	if err != nil {
...
func updateDescriptors(id string, oldD, newD []interface{}, conn *waf.WAF) error {
	wr := newWafRetryer(conn, "global")
	_, err := wr.RetryWithToken(func(token *string) (interface{}, error) {
		req := &waf.UpdateIPSetInput{
			ChangeToken: token,
			IPSetId:     aws.String(id),
			Updates:     diffDescriptors(oldD, newD),
		}
...
func diffDescriptors(oldD, newD []interface{}) []*waf.IPSetUpdate {
	updates := make([]*waf.IPSetUpdate, 0)

	for _, od := range oldD {
		descriptor := od.(map[string]interface{})
		t := descriptor["type"].(string)
		v := descriptor["value"].(string)

		if idx, contains := containsDescriptor(newD, t, v); contains {
...
func containsDescriptor(l []interface{}, dType, dValue string) (int, bool) {
	for i, d := range l {
